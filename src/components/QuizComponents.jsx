...
export function ResultPanel({ score, questionCount, percent, resultTone }) {
  const resultMeta = percent >= 80 ? 'BỨT PHÁ' : percent >= 65 ? 'VỮNG VÀNG' : percent >= 50 ? 'ĐANG TIẾN BỘ' : percent >= 30 ? 'CẦN CỐ GẮNG' : 'BẮT ĐẦU LẠI'
  return <div className={`result-panel ${resultTone}`}><div className="result-score"><span>{score}</span><small>/ {questionCount} câu đúng</small></div><div className="result-message"><span className="result-badge"><i className={`result-icon result-icon-${resultTone}`} aria-hidden="true" />{resultMeta}</span><p className="eyebrow">HOÀN THÀNH ĐỀ</p><h3>{percent >= 80 ? 'Ái chè chè, giỏi thí nhỉ.' : percent >= 65 ? 'Cũng ngon ngon đoá.' : percent >= 50 ? 'Cùi mía z, chưa tày âu.' : percent >= 30 ? 'Èo ôi seo kém quá z.' : 'Hu hu như này mà đi thi thì chít.'}</h3><p>Đáp án đã được chấm. Bạn có thể xem lại từng câu bên dưới.</p></div><div className="result-percent">{percent}%</div></div>
}
