...
function Part7QuickContent({ group, answers, checked, onAnswer }) {
  return <div className="part7-quick-list">{group.questions.map(({ question, index }) => { const answer = answers[question.id]; const correct = answer === question.answer; return <article className="part7-question" key={question.id}><div className="part7-question-heading"><span>CÂU {question.number}</span><span>ĐỌC HIỂU</span></div><p className="question-text">{question.prompt}</p><div className="options">{question.options.map((option, optionIndex) => <button className={`option ${answer === optionIndex ? 'active' : ''} ${checked && optionIndex === question.answer ? 'correct-option' : ''} ${checked && answer === optionIndex && !correct ? 'wrong-option' : ''}`} key={option} onClick={() => !checked && onAnswer(question.id, optionIndex)} disabled={checked}><span className="option-letter">{String.fromCharCode(65 + optionIndex)}</span><span>{option}</span>{answer === optionIndex && !checked && <span className="check">✓</span>}</button>)}</div>{checked && <div className={`quick-result ${correct ? 'is-correct' : 'is-wrong'}`}><span className="quick-result-icon">{correct ? '✓' : '!'}</span><div><strong>{correct ? 'Chính xác' : 'Chưa đúng'}</strong><small>Đáp án đúng: <b>{String.fromCharCode(65 + question.answer)}. {question.options[question.answer]}</b></small></div></div>}</article> })}</div>
}
...
export function ResultPanel({ score, questionCount, percent, resultTone }) {
  const resultMeta = score === questionCount ? 'HOÈN HẺO' : percent >= 80 ? 'QUÁ ĐÁ' : percent >= 65 ? 'ỔN ĐOÁ' : percent >= 50 ? 'CŨNG CŨNG I' : percent >= 30 ? 'MỊE PỊA QUÁ ' : 'THUI NÀM ĂN GÌ NỰA'
  const resultMessage = score === questionCount ? 'ÔI CHỒI ÔI LÀ CHỒI GIỎI QUÁ III' : percent >= 80 ? 'Ái chè chè, giỏi thí nhỉ.' : percent >= 65 ? 'Cũng ngon ngon đoá.' : percent >= 50 ? 'Cùi mía z, chưa tày âu.' : percent >= 30 ? 'Èo ôi seo kém quá z.' : 'Hu hu như này mà đi thi thì chít.'
  return <div className={`result-panel ${resultTone}`}><div className="result-score"><span>{score}</span><small>/ {questionCount} câu đúng</small></div><div className="result-message"><span className="result-badge"><i className={`result-icon result-icon-${resultTone}`} aria-hidden="true" />{resultMeta}</span><p className="eyebrow">HOÀN THÀNH ĐỀ</p><h3>{resultMessage}</h3><p>Xem lại đáp án bên dưới i</p></div><div className="result-percent">{percent}%</div></div>
}

function Part7QuestionGroup({ group, selectedAnswers, onAnswerAt }) {
  return <div className="part7-question-list">{group.questions.map(({ question: groupQuestion, index }) => <article className="part7-question" key={groupQuestion.id}><div className="part7-question-heading"><span>CÂU {groupQuestion.number}</span><span>ĐỌC HIỂU</span></div><p className="question-text">{groupQuestion.prompt}</p><div className="options">{groupQuestion.options.map((option, optionIndex) => <button className={`option ${selectedAnswers[index] === optionIndex ? 'active' : ''}`} key={option} onClick={() => onAnswerAt(index, optionIndex)}><span className="option-letter">{String.fromCharCode(65 + optionIndex)}</span><span>{option}</span>{selectedAnswers[index] === optionIndex && <span className="check">✓</span>}</button>)}</div></article>)}</div>
}

export function QuestionMap({ questions, groups, current, selectedAnswers, answeredCount, onCurrentChange }) {
  return <aside className="question-map"><div className="map-heading"><strong>Danh sách câu</strong><span>{answeredCount}/{questions.length}</span></div><div className="map-grid">{questions.map((item, index) => { const groupIndex = groups.findIndex((group) => group.questions.some(({ index: questionIndex }) => questionIndex === index)); return <button className={`map-group-${groupIndex % 6} ${index === current ? 'current' : ''} ${selectedAnswers[index] !== undefined ? 'answered' : ''}`} key={item.id} onClick={() => onCurrentChange(index)}>{item.number}</button> })}</div><div className="map-legend"><span><i className="legend-dot answered-dot" /> Đã chọn</span><span><i className="legend-dot" /> Chưa chọn</span></div></aside>
}

function Part7FullView({ group, groupIndex, groupCount, questionGroups, current, selectedSet, selectedAnswers, answeredCount, onAnswerAt, onCurrentChange, onSubmit }) {
  return <><div className="progress-track"><span style={{ width: `${Math.round(((groupIndex + 1) / groupCount) * 100)}%` }} /></div><div className="quiz-layout"><div className="question-card part7-card"><div className="question-label"><span>NHÓM CÂU {String(groupIndex + 1).padStart(2, '0')}</span><span className="question-type">Các câu hỏi cùng bài đọc</span></div><Part7QuestionGroup group={group} selectedAnswers={selectedAnswers} onAnswerAt={onAnswerAt} /><div className="quiz-nav"><button className="nav-button secondary" disabled={groupIndex === 0} onClick={() => onCurrentChange('previous')}>← Trước</button><button className="nav-button primary" onClick={() => onCurrentChange('next')}>{groupIndex === groupCount - 1 ? 'Xem lại' : 'Nhóm tiếp theo'} <span>→</span></button></div></div><QuestionMap questions={selectedSet.questions} groups={questionGroups} current={current} selectedAnswers={selectedAnswers} answeredCount={answeredCount} onCurrentChange={onCurrentChange} /></div><div className="submit-row"><p>Đã chọn <strong>{answeredCount}</strong>/{selectedSet.questions.length} câu</p><button className="submit-button" disabled={answeredCount === 0} onClick={onSubmit}>Nộp bài <span>↗</span></button></div></>
}

export function FullQuizView({ activePart, question, current, currentPassage, currentPassageIndex, passageGroups, questionGroup, currentGroupIndex, questionGroups, selectedSet, selectedAnswers, currentAnswer, answeredCount, isSubmitted, score, percent, resultTone, onAnswer, onAnswerAt, onCurrentChange, onSubmit }) {
  if (isSubmitted) return <><ResultPanel score={score} questionCount={selectedSet.questions.length} percent={percent} resultTone={resultTone} />{resultTone === 'perfect' && <PerfectCelebration />}</>
  if (activePart.id === 'part-7') return <Part7FullView group={questionGroup} groupIndex={currentGroupIndex} groupCount={questionGroups.length} questionGroups={questionGroups} current={current} selectedSet={selectedSet} selectedAnswers={selectedAnswers} answeredCount={answeredCount} onAnswerAt={onAnswerAt} onCurrentChange={onCurrentChange} onSubmit={onSubmit} />
...
export function ReviewList({ questions, selectedAnswers }) {
  return <div className="review-list"><div className="review-heading"><p className="eyebrow">TỔNG QUAN</p><h3>Kiểm tra lại câu trả lời</h3></div>{questions.map((item, index) => { const correct = selectedAnswers[index] === item.answer; return <div className={`review-item ${correct ? 'correct' : 'incorrect'}`} key={index}><span className="review-no">{String(index + 1).padStart(2, '0')}</span><span className="review-copy"><strong>{item.prompt}</strong><small>Bạn chọn: {selectedAnswers[index] !== undefined ? item.options[selectedAnswers[index]] : 'Chưa chọn'} · Đáp án: {item.options[item.answer]}</small></span><span className="review-status">{correct ? 'Đúng' : 'Sai'}</span></div> })}</div>
}
