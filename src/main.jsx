...
import { FullQuizView, Header, Hero, ModePicker, PartPicker, QuestionMap, QuickQuizView, QuickScope, ReviewList, SetPicker } from './components/QuizComponents'
import './styles.css'
...
function groupPart7Questions(questions) { const groups = []; questions.forEach((question, index) => { const existing = groups.find((group) => group.id === question.groupId); if (existing) existing.questions.push({ question, index }); else groups.push({ id: question.groupId, title: question.groupTitle, questions: [{ question, index }] }) }); return groups }
function getPassagePool(part) { return part.sets.flatMap((set) => groupPassages(set.questions).map((passage) => ({ ...passage, set, part }))) }
function getPart7GroupPool(part) { return part.sets.flatMap((set) => groupPart7Questions(set.questions).map((group) => ({ ...group, set, part }))) }
...
function App() {
  ...
  const quickPassagePool = activePart.id === 'part-6' ? getPassagePool(activePart) : activePart.id === 'part-7' ? getPart7GroupPool(activePart) : []
  ...
  const questionGroups = activePart.id === 'part-7' ? groupPart7Questions(selectedSet.questions) : []
  const currentGroupIndex = activePart.id === 'part-7' ? Math.max(0, questionGroups.findIndex((group) => group.questions.some((item) => item.index === current))) : 0
  const currentQuestionGroup = questionGroups[currentGroupIndex]
  ...
  const quickReady = activePart.id === 'part-6' || activePart.id === 'part-7' ? Boolean(quickPassage && quickPassage.questions.every((item) => quickAnswers[item.question.id] !== undefined)) : quickAnswer !== null
  ...
  function choosePart(part) { if (!part.available) return; setSelectedPartId(part.id); setSelectedSetId(part.sets[0]?.id || ''); setCurrent(0); setQuickQuestion(getRandomQuestion(getQuestionPool(part))); setQuickPassage(part.id === 'part-6' ? getRandomPassage(getPassagePool(part)) : part.id === 'part-7' ? getRandomPassage(getPart7GroupPool(part)) : null); setQuickAnswer(null); setQuickAnswers({}); setQuickChecked(false) }
  function chooseMode(nextMode) { setMode(nextMode); if (nextMode === 'quick') { setQuickQuestion(getRandomQuestion(quickPool)); setQuickPassage(activePart.id === 'part-6' || activePart.id === 'part-7' ? getRandomPassage(quickPassagePool) : null); setQuickAnswer(null); setQuickAnswers({}); setQuickChecked(false) } }
  ...
  function nextQuickQuestion() { if (activePart.id === 'part-6' || activePart.id === 'part-7') { setQuickPassage((previous) => getRandomPassage(quickPassagePool, previous)); setQuickAnswers({}); setQuickChecked(false); return } setQuickQuestion((previous) => getRandomQuestion(quickPool, previous)); setQuickAnswer(null); setQuickChecked(false) }
  function changeCurrent(action) {
    if (typeof action === 'number') return setCurrent(action)
    if (activePart.id === 'part-7') return setCurrent(action === 'previous' ? questionGroups[currentGroupIndex - 1].questions[0].index : currentGroupIndex === questionGroups.length - 1 ? questionGroups[currentGroupIndex].questions[0].index : questionGroups[currentGroupIndex + 1].questions[0].index)
  ...
  return <div className={`app-shell theme-${theme}`}>
    ...
    <main id="top" className="content-wrap">
      ...
      <section className="workspace" aria-label="Khu vực làm bài">
        <div className="workspace-head"><div><p className="eyebrow">CHẾ ĐỘ HỌC</p><h2>{mode === 'full' ? (isSubmitted ? 'Kết quả của bạn' : 'Làm trọn một đề') : 'Luyện nhanh từng câu'}</h2></div><div className="workspace-actions">{mode === 'full' && isSubmitted && <button className="text-button" onClick={resetSet}>Làm lại đề</button>}{mode === 'full' ? <span className="question-counter">{activePart.id === 'part-6' ? `${String(currentPassageIndex + 1).padStart(2, '0')} / ${String(passageGroups.length).padStart(2, '0')} đoạn` : activePart.id === 'part-7' ? `${String(currentGroupIndex + 1).padStart(2, '0')} / ${String(questionGroups.length).padStart(2, '0')} nhóm` : `${String(current + 1).padStart(2, '0')} / ${String(selectedSet.questions.length).padStart(2, '0')}`}</span> : <span className="question-counter">{quickAttempts} câu đã check</span>}</div></div>
         {mode === 'quick' ? <QuickQuizView activePart={activePart} quickQuestion={quickQuestion} quickPassage={quickPassage} quickAnswers={quickAnswers} quickAnswer={quickAnswer} quickChecked={quickChecked} quickReady={quickReady} onAnswer={chooseQuickAnswer} onSingleAnswer={setQuickAnswer} onCheck={checkQuickAnswer} onNext={nextQuickQuestion} /> : <>{activePart.id === 'part-6' && !isSubmitted && <QuestionMap questions={selectedSet.questions} groups={passageGroups} current={current} selectedAnswers={selectedAnswers} answeredCount={answeredCount} onCurrentChange={changeCurrent} />}<FullQuizView activePart={activePart} question={question} current={current} currentPassage={currentPassage} currentPassageIndex={currentPassageIndex} passageGroups={passageGroups} questionGroup={currentQuestionGroup} currentGroupIndex={currentGroupIndex} questionGroups={questionGroups} selectedSet={selectedSet} selectedAnswers={selectedAnswers} currentAnswer={selectedAnswers[current]} answeredCount={answeredCount} isSubmitted={isSubmitted} score={score} percent={percent} resultTone={resultTone} onAnswer={chooseAnswer} onAnswerAt={chooseAnswerAt} onCurrentChange={changeCurrent} onSubmit={submitSet} /></>}
